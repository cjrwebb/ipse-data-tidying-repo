...
cin13_21 <- cin13_21 %>% 
  ...
  select(time_period, old_la_code, new_la_code, la_name, 
         Anypoint_episodes, Started_episodes, At31_episodes, Ended_episodes) %>%
  clean_names()
...
cin13_21 <- cin13_21 %>% mutate_at(vars(anypoint_episodes:ended_episodes), ~parse_number(.))
cin_12 <-    cin_12    %>% mutate_at(vars(anypoint_episodes:ended_episodes), ~parse_number(.))
cin_11 <-   cin_11   %>% mutate_at(vars(anypoint_episodes:ended_episodes), ~parse_number(.))
cin_10 <-   cin_10   %>% mutate_at(vars(anypoint_episodes:ended_episodes), ~parse_number(.))
...
cin_22 <- cin_22 %>% 
  ...
  select(time_period, old_la_code, new_la_code, la_name, 
         Anypoint_episodes, Started_episodes, At31_episodes, Ended_episodes) %>%
  clean_names() %>% 
  mutate_at(vars(anypoint_episodes:ended_episodes), as.numeric) # Hackney missing
...
cin_dat <- cin_dat %>%
  ...
  summarise(old_la_code = first(old_la_code),
            ...
            ended_episodes = sum(ended_episodes),
...
cin_dat <- cin_dat %>%
  mutate(
    ...
    end_cin_ep_rate_10000 = (ended_episodes / population_0_16)*10000,
...
cpp_13_21 <- cpp_13_21 %>% 
  ...
  select(time_period, old_la_code, new_la_code, la_name, 
         ...
         at31_cpp = CPP_At31,
         cpp_end = CPP_end) %>%
  mutate_at(vars(cpp_start:cpp_end), ~parse_number(.))
...
cpp_12 <- cpp_12 %>%
  filter(geog_l == "LA") %>%
  select(old_la_code = geog_c, la_name = geog_n, cpp_start = CPPStart1112, at31_cpp = CPPAt310312, cpp_end = CPPCeased1112) %>%
...
cpp_11 <- cpp_11 %>%
  ...
  select(old_la_code = geog_c, la_name = geog_n, cpp_start = CPPIn1011, at31_cpp = CPPAt310311, cpp_end = CPPCeased1011) %>%
...
cpp_11 <- cpp_11 %>%
  ...
  mutate_at(vars(cpp_start:cpp_end), ~as.numeric(.)) %>%
  group_by(time_period, old_la_code, la_name) %>%
  summarise(cpp_start = sum(cpp_start),
            at31_cpp = sum(at31_cpp),
            cpp_end = sum(cpp_end)
...
cpp_10 <- cpp_10 %>%
  filter(geog_l == "LA") %>%
  # Note: In 2011 the CPP "In" category has the same description as CPP start, so assumed to be the same
  select(old_la_code = geog_c, la_name = geog_n, cpp_start = CPPIn0910, at31_cpp = CPPAt310310, cpp_end = CPPCeased0910) %>%
...
merged_data <- merged_data %>%
  mutate(cpp_start_rate10000 = (cpp_start / population_0_16)*10000,
         ...
         cpp_end_rate10000  = (cpp_end  / population_0_16)*10000
  )
...
merged_data <- merged_data %>%
  relocate(
    ...
    # CIN
    anypoint_episodes:ended_episodes, any_cin_ep_rate_10000:end_cin_ep_rate_10000,
    # CPP
    cpp_start:cpp_end_rate10000,
    # CLA
...
var_labels <- c(
  ...
  "Number of episodes of children in need that started in the year (CIN Census)",
  "Number of episodes of children in need on 31st March (CIN Census)",
  "Number of episodes of children in need that ended in the year (CIN Census)",
  ...
  "Rate of episodes of children in need that ended in the year (Rate per 10,000 0-16 population) (CIN Census)",
  
  # CPP
  "Number of Child Protection Plans starting in year (CIN Census)",
  "Number of Child Protection Plans on 31st March (CIN Census)",
  "Number of Child Protection Plans ending in year (CIN Census)",
  "Rate of Child Protection Plans starting in year (Rate per 10,000 0-16 population) (CIN Census)",
  "Rate of Child Protection Plans on 31st March (Rate per 10,000 0-16 population) (CIN Census)",
  "Rate of Child Protection Plans ending in year (Rate per 10,000 0-16 population) (CIN Census)",
...
# add CIN/CPP/CLA breakdown by ethnicity, age, gender, category of need?

cla_char1822 <- read_csv("data/ssda903-cla/children-looked-after-in-england-including-adoptions_2022/data/la_cla_on_31_march_by_characteristics.csv")

unique(cla_char1822$cla_group)
unique(cla_char1822$geographic_level)

cla_char1822 <- cla_char1822 %>%
  filter(geographic_level == "Local authority") %>%
  filter(cla_group %in% c("Age group", "Ethnicity", "Gender", "Category of need")) %>%
  filter(!characteristic %in% c("Total")) %>%
  mutate(number = parse_number(number)) %>%
  select(-time_identifier, -geographic_level, -country_code, -country_name, -percentage)

cla_age1822 <- cla_char1822 %>%
  filter(cla_group == "Age group") %>%
  pivot_wider(names_from = characteristic, values_from = number, names_prefix = "cla_age") %>%
  janitor::clean_names() %>%
  select(-cla_group)

cla_eth1822 <- cla_char1822 %>%
  filter(cla_group == "Ethnicity") %>%
  pivot_wider(names_from = characteristic, values_from = number, names_prefix = "cla_ethnicity") %>%
  janitor::clean_names() %>%
  select(-cla_group) %>%
  rename(cla_ethnicity_asian = 7, cla_ethnicity_black = 8, cla_ethnicity_mixed = 9, cla_ethnicity_other = 10, cla_ethnicity_unknown = 11, cla_ethnicity_white = 12)


sum(is.na(cla_eth1822$cla_ethnicity_black)) / length(cla_eth1822$cla_ethnicity_black)
# 25% missing; will need some sensitive imputation with priors that value must be between 1 to 5 and a real number

cla_gend1822 <- cla_char1822 %>%
  filter(cla_group == "Gender") %>%
  pivot_wider(names_from = characteristic, values_from = number, names_prefix = "cla_gender") %>%
  janitor::clean_names() %>%
  select(-cla_group)

cla_need1822 <- cla_char1822 %>%
  filter(cla_group == "Category of need") %>%
  pivot_wider(names_from = characteristic, values_from = number, names_prefix = "cla_need") %>%
  janitor::clean_names() %>%
  select(-cla_group)

# join data

cla_char1822 <- left_join(cla_age1822, cla_eth1822, by = c("time_period", "region_code", "region_name", "old_la_code", "la_name", "new_la_code")) %>%
  left_join(cla_gend1822, by = c("time_period", "region_code", "region_name", "old_la_code", "la_name", "new_la_code")) %>%
  left_join(cla_need1822, by = c("time_period", "region_code", "region_name", "old_la_code", "la_name", "new_la_code"))



# CLA 2017

cla_char17 <- read_csv("data/ssda903-cla/SFR50_2017_UnderlyingData/SFR50_CLA2017.csv")

cla_char17 <- cla_char17 %>%
  select(
    LA_order:geog_n, CLA_male:CLA_Oth, 
  )

cla_char17

# only start cla for pre 2017 (inc) but at 31 for 2018-22

cla_need17 <- read_csv("data/ssda903-cla/SFR50_2017_UnderlyingData/SFR50_ADM2017.csv")

cla_need17 <- cla_need17 %>%
  select(LA_order:geog_n, SCLA_AbNeg:SCLA_AbsPar)

cla_char17 <- left_join(cla_char17 %>% select(-LA_order), cla_need17 %>% select(-LA_order), by = c("New_geog_code", "geog_l", "geog_c", "geog_n")) 

names(cla_char17)
names(cla_char1822)

cla_char17 <- cla_char17 %>%
  mutate(time_period = 2017, .before = New_geog_code) %>%
  rename(
    new_la_code = New_geog_code, 
    region_name = geog_l, 
    region_code = geog_c, 
    la_name = geog_n,
    cla_age1_to_4_years = CLA_1to4,
    cla_age10_to_15_years = CLA_10to15,
    cla_age16_years_and_over = CLA_16over,
    cla_age5_to_9_years = CLA_5to9,
    cla_age_under_1_year = CLA_U1,
    cla_ethnicity_asian = CLA_Asian,
    cla_ethnicity_black = CLA_Black,
    cla_ethnicity_mixed = CLA_Mixed,
    cla_ethnicity_other = CLA_EOTH,
    cla_ethnicity_unknown = CLA_Oth,
    cla_ethnicity_white = CLA_White,
    cla_gender_female = CLA_female,
    cla_gender_male = CLA_male,
    scla_need_n1_abuse_or_neglect = SCLA_AbNeg,
    scla_need_n2_childs_disability = SCLA_Cdisab,
    scla_need_n3_parental_illness_or_disability = SCLA_ParIll,
    scla_need_n4_family_in_acute_stress = SCLA_FAcSt,
    scla_need_n5_family_dysfunction = SCLA_FD,
    scla_need_n6_socially_unacceptable_behaviour = SCLA_SUB,
    scla_need_n7_low_income = SCLA_LI,
    scla_need_n8_absent_parenting = SCLA_AbsPar
  ) %>%
  filter(region_name == "LA")


cla_char17 <- cla_char17 %>%
  mutate_at(vars(cla_gender_male:scla_need_n8_absent_parenting), parse_number)

cla_char17

# CLA characteristics 2016

cla_char16 <- read_csv("data/ssda903-cla/SFR41_2016_UD/SFR41_CLA2016.csv")

cla_char16 <- cla_char16 %>%
  select(
    New_geog_code:geog_n, CLA_male:CLA_Oth, 
  )

cla_char16

# only start cla for pre 2016 (inc) but at 31 for 2018-22

cla_need16 <- read_csv("data/ssda903-cla/SFR41_2016_UD/SFR41_ADM2016.csv")

cla_need16 <- cla_need16 %>%
  select(New_geog_code:geog_n, SCLA_AbNeg:SCLA_AbsPar)

cla_char16 <- left_join(cla_char16, cla_need16, by = c("New_geog_code", "geog_l", "geog_c", "geog_n")) 

names(cla_char16)
names(cla_char1822)

cla_char16 <- cla_char16 %>%
  mutate(time_period = 2016, .before = New_geog_code) %>%
  rename(
    new_la_code = New_geog_code, 
    region_name = geog_l, 
    region_code = geog_c, 
...
cla_char16 <- cla_char16 %>%
  mutate_at(vars(cla_gender_male:scla_need_n8_absent_parenting), as.numeric)

cla_char16


# CLA characteristics 2015


cla_char15 <- read_csv("data/ssda903-cla/SFR34_2015_UnderlyingData/SFR34_CLA2015.csv")

cla_char15 <- cla_char15 %>%
  select(
    New_geog_code:geog_n, CLA_male:CLA_Oth, 
  )
...
# only start cla for pre 2015 (inc) but at 31 for 2018-22

cla_need15 <- read_csv("data/ssda903-cla/SFR34_2015_UnderlyingData/SFR34_ADM2015.csv")

cla_need15 <- cla_need15 %>%
  select(New_geog_code:geog_n, SCLA_AbNeg:SCLA_AbsPar)

cla_char15 <- left_join(cla_char15, cla_need15, by = c("New_geog_code", "geog_l", "geog_c", "geog_n")) 

names(cla_char15)
names(cla_char1822)

cla_char15 <- cla_char15 %>%
  mutate(time_period = 2015, .before = New_geog_code) %>%
  rename(
    new_la_code = New_geog_code, 
    region_name = geog_l, 
    region_code = geog_c, 
...
cla_char14 <- read_csv("data/ssda903-cla/UD_SFR36_2014/SFR36_CLA2014.csv")

cla_char14 <- cla_char14 %>%
  select(
    New_geog_code:geog_n, CLA_male:CLA_Oth, 
  )
...
cla_need14 <- read_csv("data/ssda903-cla/UD_SFR36_2014/SFR36_ADM2014.csv")

cla_need14 <- cla_need14 %>%
  select(New_geog_code:geog_n, SCLA_AbNeg:SCLA_AbsPar)

cla_char14 <- left_join(cla_char14, cla_need14, by = c("New_geog_code", "geog_l", "geog_c", "geog_n")) 

names(cla_char14)
names(cla_char1822)

cla_char14 <- cla_char14 %>%
  mutate(time_period = 2014, .before = New_geog_code) %>%
  rename(
    new_la_code = New_geog_code, 
    region_name = geog_l, 
    region_code = geog_c, 
...
cla_char13 <- read_csv("data/ssda903-cla/SFR36_UnderlyingData/SFR36_CLA2013.csv")

cla_char13 <- cla_char13 %>%
  select(
    New_geog_code:geog_n, CLA_male:CLA_Oth, 
  )
...
cla_need13 <- read_csv("data/ssda903-cla/SFR36_UnderlyingData/SFR36_ADM2013.csv")

cla_need13 <- cla_need13 %>%
  select(New_geog_code:geog_n, SCLA_AbNeg:SCLA_AbsPar)

cla_char13 <- left_join(cla_char13, cla_need13, by = c("New_geog_code", "geog_l", "geog_c", "geog_n")) 

names(cla_char13)
names(cla_char1822)

cla_char13 <- cla_char13 %>%
  mutate(time_period = 2013, .before = New_geog_code) %>%
  rename(
    new_la_code = New_geog_code, 
    region_name = geog_l, 
    region_code = geog_c, 
...
cla_char12 <- read_csv("data/ssda903-cla/CLA-2012/SFR20_CLA2012.csv")

cla_char12 <- cla_char12 %>%
  select(
    New_geog_code:geog_n, CLA_male:CLA_Oth, 
  )
...
cla_need12 <- read_csv("data/ssda903-cla/CLA-2012/SFR20_ADM2012.csv")

cla_need12 <- cla_need12 %>%
  select(New_geog_code:geog_n, SCLA_AbNeg:SCLA_AbsPar)

cla_char12 <- left_join(cla_char12, cla_need12, by = c("New_geog_code", "geog_l", "geog_c", "geog_n")) 

names(cla_char12)
names(cla_char1822)

cla_char12 <- cla_char12 %>%
  mutate(time_period = 2012, .before = New_geog_code) %>%
  rename(
    new_la_code = New_geog_code, 
    region_name = geog_l, 
    region_code = geog_c, 
...
cla_char11 <- read_csv("data/ssda903-cla/underlying_20data_20sfr212011/SFR21_CLA.csv")

cla_char11 <- cla_char11 %>%
  select(
    New_geog_code:geog_n, CLA_male:CLA_Oth, 
  )
...
cla_need11 <- read_csv("data/ssda903-cla/underlying_20data_20sfr212011/SFE21_ADM.csv")

cla_need11 <- cla_need11 %>%
  select(New_geog_code:geog_n, SCLA_AbNeg:SCLA_AbsPar)

cla_char11 <- left_join(cla_char11, cla_need11, by = c("New_geog_code", "geog_l", "geog_c", "geog_n")) 

names(cla_char11)
names(cla_char1822)

cla_char11 <- cla_char11 %>%
  mutate(time_period = 2011, .before = New_geog_code) %>%
  rename(
    new_la_code = New_geog_code, 
    region_name = geog_l, 
    region_code = geog_c, 
...
cla_char1117 <- bind_rows(cla_char11, cla_char12, cla_char13, cla_char14, cla_char15, cla_char16, cla_char17)

# add 2018-22

names(bind_rows(cla_char1117, cla_char1822))

anti_join(cla_char1822, cla_char1117, by = c("new_la_code")) %>%
  group_by(new_la_code, la_name) %>%
  summarise(first(new_la_code), first(la_name))

# update codes, newer ones on older data, Northamptonshire old code in newer data
# combine Bournemouth and Poole

cla_char1117 <- cla_char1117 %>%
  mutate(
    new_la_code = case_when(new_la_code == "E06000048" ~ "E06000057",
                            new_la_code == "E06000028" ~ "E06000058",
                            new_la_code == "E06000029" ~ "E06000058",
    ...
    la_name = case_when(new_la_code == "E06000057" ~ "Northumberland",
                        new_la_code == "E06000058" ~ "Bournemouth, Christchurch and Poole",
                        new_la_code == "E06000059" ~ "Dorset",
                        new_la_code == "E06000060" ~ "Buckinghamshire",
                        new_la_code == "E08000037" ~ "Gateshead",
                        TRUE ~ la_name
                        )
  ) %>%
  group_by(time_period, region_code, region_name, la_name, new_la_code) %>%
  summarise_at(vars(cla_gender_male:scla_need_n8_absent_parenting), sum) %>%
  ungroup() 


# 2018-22 - update herefordshire names to be consistent, merged North Northamptonshire and West Northamptonshire
...
cla_char1822 <- cla_char1822 %>%
  ...
  group_by(time_period, region_code, region_name, la_name, new_la_code) %>%
  summarise_at(vars(cla_age1_to_4_years:cla_need_n8_absent_parenting), sum) %>% # sum for merged LAs
  ungroup() %>%
  mutate(
    new_la_code = case_when(new_la_code == "E10000009" ~ "E06000059",
                            TRUE ~ new_la_code
    ),
    la_name = case_when(new_la_code == "E06000019" ~ "Herefordshire",
                        
                        TRUE ~ la_name
    )
  ) %>%
  group_by(time_period, region_code, region_name, la_name, new_la_code) %>%
  summarise_at(vars(cla_age1_to_4_years:cla_need_n8_absent_parenting), ~first(na.omit(.))) %>% # first NA omit for duplicates
  ungroup() 


bind_rows(cla_char1822, cla_char1117) %>%
  group_by(new_la_code, la_name) %>%
  .$la_name %>% unique(.) %>% sort()

cla_char1117 <- cla_char1117 %>%
  mutate(
    la_name = ifelse(la_name == "Kingston Upon Hull, City of", "Kingston upon Hull, City of", la_name)
  )

bind_rows(cla_char1822, cla_char1117) %>%
  group_by(new_la_code, la_name)

cla_char <- bind_rows(cla_char1822, cla_char1117)

cla_char


# add population breakdown by ethnicity and age, age, gender

abpe <- readxl::read_xlsx("data/child-population/population-estimates-by-admin/apbefromdpmfeb23.xlsx", sheet = 2)

# merge ages into age groups
abpe

# age groups
abpe_ageg <- abpe %>%
  filter(age <=17) %>%
  group_by(ladcode21, laname_21, time, age) %>%
  summarise_at(vars(mean:upper), sum) %>%
  ungroup()
...
abpe_ageg <- abpe_ageg %>%
  mutate(
    age_group = case_when(age == 0 ~ "pop_age_u1",
                          age >= 1 & age <=4 ~ "pop_age_1to4",
                          age >= 5 & age <=9 ~ "pop_age_5to9",
                          age >= 10 & age <=15 ~ "pop_age_10to15",
                          age >= 16 ~ "pop_age_16plus"
                          )
  ) %>%
  group_by(ladcode21, laname_21, time, age_group) %>%
  summarise(
    mean = sum(mean)
  ) %>%
  ungroup() %>%
  pivot_wider(values_from = mean, names_from = age_group)

# combine to UTLA level
...
anti_join(abpe_ageg, ltla_utla_lu %>% select(-LTLA21NM, -FID),
          by = c("ladcode21" = "LTLA21CD"))


abpe_ageg <- left_join(abpe_ageg, ltla_utla_lu %>% select(-LTLA21NM, -FID),
                              by = c("ladcode21" = "LTLA21CD"))

abpe_ageg <- abpe_ageg %>%
  group_by(UTLA21CD, UTLA21NM, time) %>%
  summarise_at(
    vars(pop_age_10to15:pop_age_u1),
    sum
  ) %>% 
  ungroup()
...
abpe_sex <- abpe %>%
  filter(age <=17) %>%
  group_by(ladcode21, laname_21, time, sex) %>%
  summarise_at(vars(mean:upper), sum) %>%
  ungroup() %>%
  select(-lower, -upper) %>%
  mutate(
    sex = ifelse(sex == "Female", "popest_female", "popest_male")
  ) %>%
...
anti_join(abpe_sex, ltla_utla_lu %>% select(-LTLA21NM, -FID),
          by = c("ladcode21" = "LTLA21CD"))


abpe_sex <- left_join(abpe_sex, ltla_utla_lu %>% select(-LTLA21NM, -FID),
                       by = c("ladcode21" = "LTLA21CD"))

abpe_sex <- abpe_sex %>%
  group_by(UTLA21CD, UTLA21NM, time) %>%
  summarise_at(
    vars(popest_female:popest_male),
    sum
  ) %>% 
  ungroup()
...
# for ethnicity, take proportion by local authority from census and adjust population estimates

# weight percentage by population when updating to LA level

eth_prop_census <- readxl::read_xlsx("data/child-population/population-estimates-by-admin/2.0populationandmigrationstatisticstransformationethnicityenglandandwales2023.xlsx", 
                  sheet = 5, skip = 4) %>%
  rename(
  ...
  mutate_at(
    vars(asian:other), ~./100
  )
  
eth_prop_census <- eth_prop_census %>%
  filter(age == "Aged 15 years and under")

eth_prop_census

unique(eth_prop_census$age)

# merge ages into age groups
abpe_age15under <- abpe %>%
  filter(age <= 15) %>%
  group_by(ladcode21, laname_21, time, sex) %>%
  summarise_at(vars(mean:upper), sum)

anti_join(abpe_age15under, eth_prop_census, by = c("ladcode21" = "lad_code", "laname_21" = "lad_name", "sex")) %>%
  group_by(ladcode21, laname_21) %>%
  summarise(first(ladcode21), first(laname_21))

eth_prop_census <- eth_prop_census %>%
  mutate(
    lad_name = case_when(lad_name == "Herefordshire" ~ "Herefordshire, County of",
                         lad_name == "Kingston upon Hull" ~ "Kingston upon Hull, City of",
                         lad_name == "Bristol" ~ "Bristol, City of",
                         TRUE ~ lad_name
                         )
  )

eth_estimates_ab <- left_join(abpe_age15under, eth_prop_census %>% select(-age), by = c("ladcode21" = "lad_code", "laname_21" = "lad_name", "sex"))

eth_estimates_ab <- eth_estimates_ab %>%
  mutate_at(
    vars(asian:other), list(ethnest = ~.*mean)
  ) %>%
  group_by(ladcode21, laname_21, time) %>%
  summarise_at(vars(asian_ethnest:other_ethnest), sum)


# Aggregate from LAD to UTLA level 
...
anti_join(eth_estimates_ab, ltla_utla_lu %>% select(-LTLA21NM, -FID),
          by = c("ladcode21" = "LTLA21CD"))


eth_estimates_ab <- left_join(eth_estimates_ab, ltla_utla_lu %>% select(-LTLA21NM, -FID),
                       by = c("ladcode21" = "LTLA21CD"))

eth_estimates_ab <- eth_estimates_ab %>%
  group_by(UTLA21CD, UTLA21NM, time) %>%
  summarise_at(
    vars(asian_ethnest:other_ethnest),
    sum
  ) %>% 
  ungroup()
...
abpe <- left_join(abpe_ageg, abpe_sex, by = c("UTLA21CD", "UTLA21NM", "time")) %>%
  left_join(eth_estimates_ab, by = c("UTLA21CD", "UTLA21NM", "time"))


# merge population data with CLA characteristics 

abpe
cla_char

anti_join(abpe, cla_char, by = c("UTLA21CD" = "new_la_code", "time" = "time_period")) %>%
  .$UTLA21CD %>% unique()

anti_join(abpe, cla_char, by = c("UTLA21CD" = "new_la_code", "time" = "time_period")) %>%
  .$UTLA21NM %>% unique()

# merge North Northamptonshire and West Northamptonshire in abpe data

abpe <- abpe %>%
  mutate(
    UTLA21CD = case_when(UTLA21CD == "E06000061" ~ "E10000021",
                         UTLA21CD == "E06000062" ~ "E10000021",
                         TRUE ~ UTLA21CD),
    UTLA21NM = case_when(UTLA21CD == "E10000021" ~ "Northamptonshire",
                         TRUE ~ UTLA21NM)
  ...
  group_by(UTLA21CD, UTLA21NM, time) %>%
  summarise_at(vars(everything()), sum) %>%
  ungroup()

anti_join(cla_char, abpe %>% select(-UTLA21NM), by = c("new_la_code" = "UTLA21CD",  "time_period" = "time"))
cla_char <- left_join(cla_char, abpe %>% select(-UTLA21NM), by = c("new_la_code" = "UTLA21CD",  "time_period" = "time"))

# add data to merged data

merged_data <- read_csv("tidied_data/merged_data.csv")

merged_data <- merged_data %>% select(names(merged_data)[1:119])

anti_join(cla_char, merged_data, by = c("new_la_code", "time_period"))

anti_join(merged_data, cla_char, by = c("new_la_code", "time_period")) %>%
  filter(time_period %in% 2011:2022)


merged_data <- left_join(merged_data, cla_char %>% select(-la_name), by = c("new_la_code", "time_period"))
...
names(merged_data)[120:162]

newvarnames <- names(merged_data)[120:162]

newvarcodes <- c("Region code",
                 "Region name",
                 # age
                 "Number of children looked after aged 1 to 4 years old (inclusive)",
                 "Number of children looked after aged 10 to 15 years old (inclusive)",
                 "Number of children looked after aged 16 years old or older",
                 "Number of children looked after aged 5 to 9 years old (inclusive)",
                 "Number of children looked after aged under 1 year old",
                 # ethnicity
                 "Number of children looked after with Asian/British Asian ethnic background",
                 "Number of children looked after with Black/Black British ethnic background",
                 "Number of children looked after with Mixed Heritage ethnic background",
                 "Number of children looked after with Other ethnic background",
                 "Number of children looked after with Unknown ethnic background",
                 "Number of children looked after with White/White British ethnic background",
                 # gender
                 "Number of children looked after who are female",
                 "Number of children looked after who are male",
                 # need
                 "Number of children looked after at March 31st whose primary need at assessment was N1 Abuse or Neglect (2018-2022 only)",
                 "Number of children looked after at March 31st whose primary need at assessment was N2 Child's Disability (2018-2022 only)",
                 "Number of children looked after at March 31st whose primary need at assessment was N3 Parental illness or disability (2018-2022 only)",
                 "Number of children looked after at March 31st whose primary need at assessment was N4 Family in acute stress (2018-2022 only)",
                 "Number of children looked after at March 31st whose primary need at assessment was N5 Family Dysfunction (2018-2022 only)",
                 "Number of children looked after at March 31st whose primary need at assessment was N6 Socially Unacceptable Behaviour (2018-2022 only)",
                 "Number of children looked after at March 31st whose primary need at assessment was N7 Low income (2018-2022 only)",
                 "Number of children looked after at March 31st whose primary need at assessment was N8 Absent parenting (2018-2022 only)",
                 # starting need (2011-2017)
                 "Number of children starting to be looked after whose primary need at assessment was N1 Abuse or Neglect (2011-2017 only)",
                 "Number of children starting to be looked after whose primary need at assessment was N2 Child's Disability (2011-2017 only)",
                 "Number of children starting to be looked after whose primary need at assessment was N3 Parental illness or disability (2011-2017 only)",
                 "Number of children starting to be looked after whose primary need at assessment was N4 Family in acute stress (2011-2017 only)",
                 "Number of children starting to be looked after whose primary need at assessment was N5 Family Dysfunction (2011-2017 only)",
                 "Number of children starting to be looked after whose primary need at assessment was N6 Socially Unacceptable Behaviour (2011-2017 only)",
                 "Number of children starting to be looked after whose primary need at assessment was N7 Low income (2011-2017 only)",
                 "Number of children starting to be looked after whose primary need at assessment was N8 Absent parenting (2011-2017 only)",
                 # population estimate subgroups from admin data
                 "Mean Population estimate for children (aged 10-15) (ONS Population Estimates from Admin Data)",
                 "Mean Population estimate for children (aged 16+) (ONS Population Estimates from Admin Data)",
                 "Mean Population estimate for children (aged 1-4) (ONS Population Estimates from Admin Data)",
                 "Mean Population estimate for children (aged 5-9) (ONS Population Estimates from Admin Data)",
                 "Mean Population estimate for children (aged under 1) (ONS Population Estimates from Admin Data)",
                 "Mean Population estimate for children (female) (ONS Population Estimates from Admin Data)",
                 "Mean Population estimate for children (male) (ONS Population Estimates from Admin Data)",
                 "Population estimate for children (Asian/British Asian) (ONS Population Estimates from Admin Data multiplied by Census 2021 0-16 ethnic proportion for LA)",
                 "Population estimate for children (Black/Black British) (ONS Population Estimates from Admin Data multiplied by Census 2021 0-16 ethnic proportion for LA)",
                 "Population estimate for children (Mixed Heritage) (ONS Population Estimates from Admin Data multiplied by Census 2021 0-16 ethnic proportion for LA)",
                 "Population estimate for children (White/White British) (ONS Population Estimates from Admin Data multiplied by Census 2021 0-16 ethnic proportion for LA)",
                 "Population estimate for children (Other ethnic group) (ONS Population Estimates from Admin Data multiplied by Census 2021 0-16 ethnic proportion for LA)"
                 )

added_vars <- tibble(var_names = newvarnames, var_labels = newvarcodes)

merged_data_codebook <- read_csv("tidied_data/merged_data_codebook.csv")

merged_data_codebook <- merged_data_codebook[1:119,]

merged_data_codebook <- bind_rows(merged_data_codebook, added_vars)

write_csv(merged_data_codebook, "tidied_data/merged_data_codebook.csv")
write_csv(merged_data, "tidied_data/merged_data.csv")
